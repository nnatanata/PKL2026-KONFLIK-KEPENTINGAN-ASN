...
class DokumenPotensialController extends Controller
{
    ...
    private function viewDocxAsHtml($dokumen, $fullPath)
    {
        try {
            ...
            return response()->view('errors.docx-preview', [
                ...
                'downloadUrl' => route('inspektorat.dokumen-potensial.download', $dokumen->id)
            ], 200);
        }
    }
    ...
    private function generateDummyPDF($dokumen, $filePath)
    {
        ...
        if ($dokumen->laporanPotensial) {
            $pdf->Cell(50, 8, 'Dugaan:', 0, 0);
            $pdf->MultiCell(0, 8, $dokumen->laporanPotensial->dugaan_konflik);
    ...
    private function generateDummyImage($dokumen, $filePath, $extension)
    {
        ...
        imagestring($image, $font, 50, 100, 'Nama File: ' . $dokumen->nama_file_potensial, $textColor);
        ...
        imagestring($image, $font, 50, 190, 'ID Laporan: #' . $dokumen->laporan_potensial_id, $textColor);
    ...
    private function generateDummyDocx($dokumen, $filePath)
    {
        ...
        if ($dokumen->laporanPotensial) {
            $section->addText('Dugaan Konflik: ' . $dokumen->laporanPotensial->dugaan_konflik);
