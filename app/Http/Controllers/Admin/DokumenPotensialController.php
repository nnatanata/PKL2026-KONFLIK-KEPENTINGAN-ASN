...
class DokumenPotensialController extends Controller
{
    //download dokumen
    public function download($id)
    {
        $dokumen = DokumenPotensial::findOrFail($id);
        
        //path file dummy
        $filePath = 'dokumen_potensial/' . $dokumen->nama_file_potensial;
        ...
        return Response::download($fullPath, $dokumen->nama_file_potensial);
    }
    ...
    public function view($id)
    {
        ...
        $fullPath = Storage::disk('public')->path($filePath);
        $extension = strtolower(pathinfo($dokumen->nama_file_potensial, PATHINFO_EXTENSION));
        ...
        //docx konversi ke HTML buat preview
        if ($extension === 'docx') {
        ...
        return Response::file($fullPath, [
            ...
            'Content-Disposition' => 'inline; filename="' . $dokumen->nama_file_potensial . '"'
        ]);
    }
    ...
    private function viewDocxAsHtml($dokumen, $fullPath)
    {
        try {
            ...
            <head>
                <meta charset="UTF-8">
                <title>' . htmlspecialchars($dokumen->nama_file_potensial) . '</title>
                <style>
                    body {
            ...
            return response()->view('errors.docx-preview', [
                ...
                'filename' => $dokumen->nama_file_potensial,
                'downloadUrl' => route('dokumen-potensial.download', $dokumen->id)
            ], 200);
        }
    }
    ...
    private function generateDummyFile($dokumen)
    {
        ...
        if (!Storage::disk('public')->exists('dokumen_potensial')) {
            Storage::disk('public')->makeDirectory('dokumen_potensial');
        }
    ...
    private function generateDummyPDF($dokumen, $filePath)
    {
        ...
        $pdf->Cell(0, 10, 'LAPORAN POTENSIAL', 0, 1, 'C');
        $pdf->Ln(10);
        ...
        $pdf->Cell(0, 8, $dokumen->nama_file_potensial, 0, 1);
        ...
        $pdf->Cell(0, 8, '#' . $dokumen->laporan_potensial_id, 0, 1);
        
        if ($dokumen->laporanPotensial) {
            $pdf->Cell(50, 8, 'Judul:', 0, 0);
            $pdf->MultiCell(0, 8, $dokumen->laporanPotensial->judul_potensial);
            $pdf->Cell(50, 8, 'Tanggal:', 0, 0);
            $pdf->Cell(0, 8, $dokumen->laporanPotensial->tanggal_potensial, 0, 1);
            $pdf->Cell(50, 8, 'Nama Terduga:', 0, 0);
            $pdf->Cell(0, 8, $dokumen->laporanPotensial->nama_terduga, 0, 1);
        }
    ...
    private function generateDummyImage($dokumen, $filePath, $extension)
    {
        ...
        imagestring($image, $font, 50, 70, 'LAPORAN POTENSIAL', $redColor);
        
        imagestring($image, $font, 50, 110, 'Nama File: ' . $dokumen->nama_file_potensial, $textColor);
        imagestring($image, $font, 50, 140, 'Tipe Dokumen: ' . $dokumen->tipe_doc, $textColor);
        imagestring($image, $font, 50, 170, 'Tanggal: ' . $dokumen->created_at->format('d F Y'), $textColor);
        imagestring($image, $font, 50, 200, 'ID Laporan: #' . $dokumen->laporan_potensial_id, $textColor);
    ...
    private function generateDummyDocx($dokumen, $filePath)
    {
        ...
        $section->addTitle('LAPORAN POTENSIAL', 2);
        $section->addTextBreak(2);
        
        $section->addTitle('Informasi Dokumen', 2);
        $section->addText('Nama File: ' . $dokumen->nama_file_potensial);
        ...
        $section->addText('ID Laporan: #' . $dokumen->laporan_potensial_id);
        
        if ($dokumen->laporanPotensial) {
            $section->addText('Judul: ' . $dokumen->laporanPotensial->judul_potensial);
            $section->addText('Tanggal: ' . $dokumen->laporanPotensial->tanggal_potensial);
            $section->addText('Nama Terduga: ' . $dokumen->laporanPotensial->nama_terduga);
        }
